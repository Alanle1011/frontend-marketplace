...
import {
    GET_LISTED_ITEM_BY_ADDRESS_TOKENID, GET_PREVIOUS_BIDDING, GET_PREVIOUS_TRANSACTION
...
const Details = () => {
    ...
    const marketplaceAddress = chainId === "0xaa36a7" ? networkMapping[`${chainId}`].NftMarketPlace[0].toString() : "0x78a10Ad00434Ce44e0ACAC9293A258EEdDdcAA51"
    const { data } = useQuery(GET_LISTED_ITEM_BY_ADDRESS_TOKENID(address, tokenId))
    const { data: dataBid } = useQuery(GET_PREVIOUS_TRANSACTION(address, tokenId))
    ...
    const handleBuyBidNft = () => {
        if (account !== nftOwner && listedNft && listedNft.endTime < currentTime) {
            buyBidItem({
                onError: (error) => console.log(error),
                onSuccess: () => handleBuyItemSuccess()
            })
        ...
        <div className="w-full p-5 px-28">
            <div className="flex gap-6">
                <div className="flex flex-col gap-2">
                    ...
                    <div className="border border-gray-500 w-full rounded-2xl p-4">
                        ...
                        {tokenDescription ?
                            ...
                            (<p className="flex items-center justify-center p-4 font-bold text-lg">There are no
                                Description</p>)
                        }
                ...
                <div className="w-full flex flex-col gap-4">
                    <h1 className=" font-bold text-4xl">{tokenName} #{tokenId}</h1>
                    <a href={`https://sepolia.etherscan.io/address/${nftOwner}`} target="_blank" rel="noreferrer">
                        <p>Owned by <span className="text-lg text-blue-500">{truncateStr(nftOwner, 12) || null}</span>
                        </p>
                    </a>
                    {!!listedNft && listedNft.isFinishedBidding === false ? (
                        <div className="border-gray-500 border bg-blue-100 rounded-lg w-full p-5 mt-5">
                            <div className="flex justify-between">
                                <div>
                                    <p>Current Price</p>
                                    <p className="text-4xl font-bold">{ethers.utils.formatUnits(listedNft ? listedNft?.price : 0, "ether")}{" "} ETH</p>
                                </div>
                                <div>
                                    <p className="text-end">Highest Bidder</p>
                                    <p className="text-lg font-medium text-end">{listedNft?.buyer === "0x0000000000000000000000000000000000000000" ? "-" : truncateStr(listedNft?.buyer, 15)}</p>
                                    <p className="text-end">End Time</p>
                                    <p className="text-lg font-medium text-end">{listedNft?.endTime === "0" ? "-" : formatDate(listedNft.endTime)}</p>
                                </div>
                            </div>
                            <div className="w-full flex gap-5 mt-10">
                                {listedNft?.seller === account ? (
                                    ...
                                    listedNft?.isBidding === false ? (
                                        ...
                                        listedNft?.endTime > currentTime || listedNft?.endTime === "0" ? (
                                            <button
                                            ...
                                            listedNft.buyer === account ? (
                                                <button
                                                    className="w-full bg-blue-400 rounded-lg p-3 text-xl font-bold hover:bg-blue-500"
                                                    disabled={isLoading || listedNft.buyer !== account}
                    ...
                    <div className="rounded-lg bg-blue-100 border-gray-500 border p-2">
                        ...
                        {listedNft?.isBidding === true && previousBidding.length > 0 ?
                            (<table className="w-full">
                                <thead className="border-b border-gray-500 w-full">
                                    <tr className="flex w-full mb-4">
                                        <th className='p-4 w-1/6'>Transaction</th>
                                ...
                                {[...previousBidding].sort((a, b) => b.blockTimestamp - a.blockTimestamp)
                                    .map((bid, key) => (
                                        <tr key={key} className=" w-full mb-4">
                                            <td className={"p-4 w-1/6"}>
                                                <p className="text-center">{truncateStr(bid.transactionHash, 10)}</p>
                                            </td>
                                            <td className={"p-4 w-1/6"}>
                                                <p className="text-center">{truncateStr(bid.buyer, 10)}</p>
                                            </td>
                                            <td className={"p-4 w-1/6"}>
                                                <p className="text-center">{ethers.utils.formatUnits(bid.price, "ether")} ETH</p>
                                            </td>
                                            <td className={"p-4 w-1/6"}>
                                                <p className="text-center">{formatDate(bid.blockTimestamp)}</p>
                                            </td>
                                            <td className={"p-4 w-1/6"}>
                                                <p className="text-center">{formatDate(bid.endTime)}</p>
                                            </td>
                                            <td className={"p-4 w-1/6"}>
                                                <p className="text-center">Raise Price</p>
                                            </td>
                                        </tr>
                        ...
                        {tokenAttributes.length ? (
                            <div className="grid grid-cols-3 max-lg:grid-cols-2 max-md:grid-cols-1 gap-x-2 gap-y-4">
                                {tokenAttributes.map((attribute, key) => {
                                    return (
                                        <div key={key} className="flex cursor-pointer items-center justify-between rounded
                                        border border-gray-500 px-4 py-2 hover:border-black hover:bg-blue-300">
                                            <div className="flex h-full flex-col justify-between text-sm">
                                                <p className="text-grays">{attribute.trait_type}</p>
                            ...
                            <div className="flex justify-center items-center w-full">
                                <p className="text-2xl font-bold">No traits Available</p>
                            </div>
                        )}
                    </div>
